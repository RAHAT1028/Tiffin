...
export const MealModal: React.FC<MealModalProps> = ({ meal, onClose, onAddToCart }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
      <div 
        className="bg-slate-900 rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-800 text-white relative animate-in zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 p-2.5 rounded-full bg-slate-950/80 hover:bg-slate-800 text-slate-300 shadow-md backdrop-blur-md border border-slate-700 transition-all hover:scale-105"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="relative h-64 sm:h-72 w-full overflow-hidden bg-slate-950">
          ...
          <div className="absolute top-4 left-4 flex gap-2">
            <span className="px-3 py-1 rounded-xl bg-slate-950/80 backdrop-blur-md text-white font-bold text-xs border border-slate-800">
              {meal.dayOfWeek}
            </span>
        ...
        <div className="p-6 sm:p-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
            <div>
              ...
              <p className="text-sm text-slate-300 mt-1 leading-relaxed">{meal.description}</p>
            </div>
            <div className="text-left sm:text-right flex-shrink-0 bg-slate-950 p-3 rounded-2xl border border-slate-800">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {meal.includedItems.map((item, idx) => (
                <div key={idx} className="flex items-center gap-2.5 p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-xs font-semibold text-slate-200">
                  <div className="w-5 h-5 rounded-full bg-orange-950 text-orange-400 border border-orange-500/30 flex items-center justify-center flex-shrink-0">
          ...
          <div className="bg-slate-950 rounded-2xl p-4 border border-slate-800">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
              <div className="bg-slate-900 p-3 rounded-xl border border-slate-800">
                ...
                <span className="text-base font-extrabold text-white">{meal.nutrition.calories} kcal</span>
              </div>
              <div className="bg-slate-900 p-3 rounded-xl border border-slate-800">
          ...
          <div className="flex flex-wrap items-center gap-2 pt-2">
            <span className="text-xs font-bold text-slate-400 mr-2">Allergen Safety:</span>
            {meal.isNutFree && (
            ...
            {meal.allergens.length === 0 ? (
              <span className="px-2.5 py-1 rounded-lg bg-emerald-950 border border-emerald-600 text-emerald-300 font-bold text-[11px]">
                Top-9 Allergen Free
              </span>
            ) : (
              meal.allergens.map((alg) => (
                <span key={alg} className="px-2.5 py-1 rounded-lg bg-rose-950/80 border border-rose-600 text-rose-300 font-semibold text-[11px] flex items-center gap-1">
          ...
          <div className="pt-4 border-t border-slate-800 flex items-center justify-end gap-3">
            <button
              onClick={onClose}
              className="px-5 py-3 rounded-xl border border-slate-700 text-slate-300 font-bold text-xs hover:bg-slate-800 transition-colors"
            >
              Close
            </button>
            <button
