      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <button
                ...
                className={`flex-1 min-w-[140px] p-3.5 rounded-2xl text-left font-bold transition-all border ${
                  isSelected
                    ...
                    : 'bg-slate-900/90 hover:bg-slate-850 text-slate-400 border-slate-800'
                }`}
              >
                <div className="flex items-center justify-between">
                  <span className={`text-xs uppercase tracking-wider font-bold ${isSelected ? 'text-orange-200' : 'text-orange-500/80'}`}>Day</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredMeals.map((meal) => (
            <div
              key={meal.id}
              className="bg-slate-900/90 rounded-3xl border border-slate-800 shadow-xl hover:shadow-2xl hover:shadow-orange-950/30 hover:border-orange-500/50 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between overflow-hidden group"
            >
              {/* Image & Badges */}
              <div>
                <div className="relative h-52 w-full overflow-hidden bg-slate-950">
                  ...
                  {/* Price Chip */}
                  <div className="absolute top-3 right-3 bg-slate-950/90 backdrop-blur-md px-3 py-1 rounded-xl text-xs font-extrabold text-orange-400 border border-orange-500/30 shadow-md">
                  ...
                  <div className="absolute bottom-3 left-3 right-3 flex flex-wrap gap-1.5">
                    {meal.tags.map((tag) => (
                      <span key={tag} className="px-2 py-0.5 rounded-md bg-slate-950/80 border border-slate-800 backdrop-blur-sm text-slate-200 text-[10px] font-semibold">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <div className="p-6">
                  ...
                  <p className="text-xs text-slate-400 mt-1.5 font-medium line-clamp-2">
                    {meal.tagline}
                  </p>
                  ...
                  <div className="mt-4 p-2.5 rounded-xl bg-slate-950 border border-slate-800/80 flex items-center justify-between text-xs text-slate-300">
              ...
              <div className="p-6 pt-0 flex items-center gap-2">
                <button
                  ...
                  className="flex-1 py-2.5 px-3 rounded-xl border border-slate-800 text-slate-300 hover:text-white hover:bg-slate-800 font-bold text-xs flex items-center justify-center gap-1.5 transition-colors"
                >
        ...
        {filteredMeals.length === 0 && (
          <div className="text-center py-16 bg-slate-900 rounded-3xl border border-dashed border-slate-800">
            ...
            <h3 className="font-bold text-slate-300 text-base">No meals match your active dietary filters.</h3>
