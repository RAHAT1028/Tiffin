...
export const MealPlanSelector: React.FC<MealPlanSelectorProps> = ({ onSelectPlan }) => {
  return (
    <section id="plans" className="py-20 bg-slate-950 text-white relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {MEAL_PLANS.map((plan) => (
            <div
              ...
              className={`rounded-3xl p-8 flex flex-col justify-between transition-all duration-300 relative ${
                plan.isPopular
                  ? 'bg-gradient-to-b from-slate-900 to-orange-950/40 shadow-2xl shadow-orange-950/40 border-2 border-orange-500 md:-translate-y-2'
                  : 'bg-slate-900/90 shadow-xl border border-slate-800 hover:border-slate-700 hover:shadow-2xl'
              }`}
            >
              {/* Popular Badge */}
              {plan.isPopular && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-orange-600 to-amber-600 text-white text-xs font-extrabold uppercase tracking-wider px-4 py-1 rounded-full shadow-lg shadow-orange-600/30 flex items-center gap-1">
              ...
              <div>
                ...
                <div className="mb-6 p-4 rounded-2xl bg-slate-950 border border-slate-800">
                  ...
                  <p className="text-[11px] text-slate-500 mt-0.5">Free hot insulated delivery included</p>
                </div>
              ...
              <button
                ...
                className={`w-full py-3.5 px-6 rounded-2xl font-bold text-sm transition-all flex items-center justify-center gap-2 shadow-sm ${
                  plan.isPopular
                    ...
                    : 'bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 hover:border-orange-500/50'
                }`}
              >
        ...
        {/* Guarantee Banner */}
        <div className="mt-12 bg-slate-900 rounded-3xl p-6 sm:p-8 border border-slate-800 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6">
